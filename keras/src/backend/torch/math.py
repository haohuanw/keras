...
def _segment_fn_transform(segment_ids, segment_fn):
    for _ in range(len(segment_ids.shape) - 1):
        segment_fn = torch.vmap(segment_fn)
    return segment_fn


def _segment_fn_1d(data, segment_ids, reduction_method, num_segments):
...
def segment_sum(data, segment_ids, num_segments=None, **kwargs):
    ...
    segment_fn = partial(
        _segment_fn_1d, reduction_method="sum", num_segments=num_segments
    )
...
def segment_max(data, segment_ids, num_segments=None, **kwargs):
    ...
    segment_fn = partial(
        _segment_fn_1d, reduction_method="amax", num_segments=num_segments
    )
    segment_fn = _segment_fn_transform(segment_ids, segment_fn)
